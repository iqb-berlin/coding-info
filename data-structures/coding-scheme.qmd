...
Diese Festlegung hat schwankende Bedeutung für die Kodierung. Sie ist dafür gedacht, die sehr komplexen Parameter eines Kodierschemas beim Editieren übersichtlicher zu gestalten. Ob bzw. wie diese Information durch den Schemer genutzt wird, ist nicht streng festgelegt.
...
* `RULES_ONLY`: Automatische Kodierung; sämtliche Optionen für manuelle Kodierung werden ausgeblendet.
...
Es besteht an vielen Stellen der Verarbeitung der Wunsch, einen Code klar den Kategorien "RICHTIG" oder "FALSCH" zuzuordnen (unabhängig von `score`). Der Parameter `type` kann die Werte `UNSET`, `FULL_CREDIT`, `PARTIAL_CREDIT` und `NO_CREDIT` annehmen. Es soll hierfür nicht (mehr) der Parameter `label` benutzt werden.
